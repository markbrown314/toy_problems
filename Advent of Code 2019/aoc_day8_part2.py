...
final_layer = [2] * (dimensions[0] * dimensions[1])
for i in range(0, len(layer_array)):
    for j in range(0, len(layer_array[i])):
        if final_layer[j] == 2 and layer_array[i][j] != 2:
            final_layer[j] = layer_array[i][j]

for i in range(0, dimensions[0] * dimensions[1]):
        if not i % dimensions[0]:
            print('')
        if final_layer[i] == 1:
            print('*', end = "")
        else:
